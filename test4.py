...
sankey_data = {
    'Source': [],
    'Target': [],
    'Value': []  # Use priority ID as the value for the Sankey flow
}
...
for issue in issues:
    ...
    priority_id = int(issue['fields']['priority']['id'])  # Assuming priority ID is suitable as a score

    # Add project to issue type
    sankey_data['Source'].append(project)
    sankey_data['Target'].append(issue_type)
    sankey_data['Value'].append(priority_id)  # Using priority ID directly
